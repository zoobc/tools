...
int main(int argc, char* argv[]) {
    ToolConfig config;
    config.name = "ZooBC Swap Offer — accept";
    config.description = "accept a swap offer by id.";
    config.tx_type = static_cast<uint32_t>(zoobc::TransactionType::AcceptSwapOffer);
    ...
    try {
        ...
        return run_transaction(params,config.tx_type,kp.Value().public_key,std::vector<uint8_t>{},body,kp.Value(),KeyType::ZBC,extra,emit_error,"SUCCESS: Swap accept submitted!");
