...
int main(int argc, char* argv[]) {
    ToolConfig config; config.name="ZooBC App — resign"; config.description="resign an app by id.";
    config.tx_type=static_cast<uint32_t>(zoobc::TransactionType::ResignApp); config.has_recipient=false;
    ...
    try { auto kp=derive_zbc_keypair(params.values[0]); if(!kp.IsOk()){emit_error(kp.GetError().ToString());return 1;}
        ...
        return run_transaction(params,config.tx_type,kp.Value().public_key,std::vector<uint8_t>{},body,kp.Value(),KeyType::ZBC,extra,emit_error,"SUCCESS: App resign submitted!");
