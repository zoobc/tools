...
int main(int argc, char* argv[]) {
    ToolConfig config;
    config.name = "ZooBC Swap Offer — cancel";
    config.description = "cancel a swap offer by id.";
    config.tx_type = static_cast<uint32_t>(zoobc::TransactionType::CancelSwapOffer);
    config.has_recipient = false;
    config.params = {
        {"Sender private key","sender_privkey","Sender private key (64 hex)","",true,nullptr},
        {"Offer id","offer_id","the swap offer id (decimal int64)","",true,nullptr},
    };
    ...
    try {
        ...
        int64_t oid=parse_id_i64(params.values[1],"offer_id");
        std::vector<uint8_t> body; putU64(body,oid);
        json extra={{"offer_id",oid}};
        return run_transaction(params,config.tx_type,kp.Value().public_key,std::vector<uint8_t>{},body,kp.Value(),KeyType::ZBC,extra,emit_error,"SUCCESS: Swap cancel submitted!");
